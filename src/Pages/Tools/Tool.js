    ...
    return (
        <div className='rounded-lg border ease-in duration-700 hover:text-white hover:shadow-2xl hover:bg-lime-700 shadow-lg p-4 m-6'>
            ...
            <div>
                ...
                <br />
                <h1><b>Price:</b> <span className='text-green-500 font-bold'>US ${price}</span></h1>
                <div className='flex justify-between'>
                    <h1>Minimum Order Quantity: <span className='text-green-500 font-bold'>{minimumQuantity} PCS</span></h1>
                    <h1>Available Quantity: <span className='text-green-500 font-bold'>{availableQuantity} PCS</span></h1>
                </div>
                <br />
                <h1><small>{description}</small></h1>
                {
                    user && admin ? <button onClick={() => navigate(`/updateProduct/${_id}`)} className="btn mt-2 btn-outline w-full ease-in-out duration-500">Edit Or Update</button> :
